        ...
        <section key={ id } className='transition duration-150 ease-out'>
            <Link to={ '/' + type +'/' + id} key={ id }>
                <div className='text-left mb-3 block relative h-32 rounded-lg drop-shadow'>
                    ...
                    <span className='relative flex justify-left  h-full items-end'><h1 className='bg-white bg-opacity-80 w-full text-base text-cyan-900 pb-1 pt-1 px-3 font-medium overflow-hidden rounded-b-md backdrop-blur'>{ name.toUpperCase() }</h1></span>
                </div>
            </Link>
        </section>
    );
